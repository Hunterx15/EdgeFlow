...
async function fullStatus() {
  const [
    ...
    servicesService
      .stats()
      .catch(() => ({ healthy: 0, unhealthy: 0, unknown: 0, total: 0 })),
    analyticsService
      .p95Latency({
    ...
    queryOne(
      "SELECT COUNT(*)::int AS c FROM api_keys WHERE enabled = TRUE AND (expires_at IS NULL OR expires_at > NOW())",
    ).catch(() => ({ c: 0 })),
    queryOne("SELECT COUNT(*)::bigint AS c FROM request_logs").catch(() => ({
      c: 0,
    })),
    analyticsService
      .latencyPercentiles({
      ...
      .catch(() => ({ p50: 0, p95: 0, p99: 0, avg: 0, max: 0 })),
  ]);
  ...
  const subsystems = {
    ...
    routeCache: {
      ...
      lastRefreshedAt: routeCache.snapshot().lastRefreshedAt || null,
    },
    node: nodeMetrics,
    live: {
  ...
  const anyDown =
    subsystems.database.status === "down" || subsystems.redis.status === "down";
  const anyDegraded = Object.values(subsystems).some(
