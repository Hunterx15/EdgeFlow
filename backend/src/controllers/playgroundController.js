...
async function send(req, res, next) {
  try {
    ...
    if (!method || !url) {
      return res.status(400).json({ success: false, error: { code: 'BAD_REQUEST', message: 'method and url are required' } });
    }
    ...
    return ok(res, { ...responsePayload, latencyMs, request: { method: method.toUpperCase(), url: targetPath, headers, body } });
  } catch (err) { next(err); }
}
