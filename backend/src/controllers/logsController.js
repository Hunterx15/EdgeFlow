...
async function list(req, res, next) {
  try {
    ...
    if (statusCode) { values.push(statusCode); where.push(`status_code >= $${values.length} AND status_code < $${values.length + 1}`); values.push(statusCode + 100); }
    ...
    const total = await queryOne(`SELECT COUNT(*)::bigint AS c FROM request_logs ${whereClause}`, values.slice(0, -2));
    return ok(res, items, paginate({ page: Math.floor(offset / limit) + 1, limit, total: Number(total?.c) || 0 }));
  } catch (err) { next(err); }
}

async function getById(req, res, next) {
  try {
    const r = await queryOne('SELECT * FROM request_logs WHERE id = $1', [req.params.id]);
    if (!r) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Log not found' } });
    return ok(res, r);
  } catch (err) { next(err); }
}
...
async function pipeline(req, res, next) {
  try {
    ...
    const r = await queryOne('SELECT request_id, method, public_path, status_code, latency_ms, pipeline_stages, created_at FROM request_logs WHERE id = $1', [req.params.id]);
    if (!r) return res.status(404).json({ success: false, error: { code: 'NOT_FOUND', message: 'Log not found' } });
    return ok(res, r);
  } catch (err) { next(err); }
}
...
async function timeline(req, res, next) {
  try {
    ...
    const items = await queryMany(
      `SELECT id, request_id, method, public_path, status_code, latency_ms, pipeline_stages, created_at
