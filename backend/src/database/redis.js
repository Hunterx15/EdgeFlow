...
class MemoryFallback {
  constructor() {
    logger.warn(
      "redis: running in MEMORY FALLBACK mode - not safe for multi-instance",
    );
  ...
  async del(k) {
    return memoryStore.delete(k) ? 1 : 0;
  }
  async incr(k) {
    const e = memoryStore.get(k) || { value: "0" };
    const n = parseInt(e.value, 10) + 1;
    memoryStore.set(k, { value: String(n), expiresAt: e.expiresAt });
    return n;
  }
  async expire(k, ttl) {
    const e = memoryStore.get(k);
...
async function getClient() {
  ...
  try {
    ...
    client = createClient({
      ...
      socket: {
        reconnectStrategy: (r) =>
          r > 10 ? new Error("REDIS_MAX_RETRIES") : Math.min(r * 200, 2000),
      },
    });
    client.on("error", (err) =>
    ...
    logger.info("REDIS CONNECTED", {
      fallback: isFallback(),
      urlConfigured: !!config.redis.url,
    });
    return client;
  } catch (err) {
    logger.error("redis: failed to connect, using memory fallback", {
      error: err.message,
    });
    fallbackMode = true;
    client = null;
...
function isFallback() {
  return fallbackMode || !config.redis.enabled;
}
...
async function close() {
  ...
  if (fallbackInstance) {
    await fallbackInstance.disconnect();
