...
export default function TimelinePage() {
  ...
  const load = useCallback(async () => {
    try {
      setLogs(await logsApi.timeline(30));
    } catch {
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">
            Live stream of recent requests with their full pipeline chain.
          </p>
        </div>
      ...
      <Card noPadding>
        {loading ? (
          ...
          <EmptyState
            ...
            description="Send a few requests through the gateway to populate the timeline."
          />
        ) : (
...
function TimelineEntry({ log }) {
  ...
  try {
    stages =
      typeof log.pipeline_stages === "string"
        ? JSON.parse(log.pipeline_stages)
        : log.pipeline_stages || [];
  } catch {
    stages = [];
  }
  ...
  return (
    <div className="px-5 py-4 hover:bg-slate-800/20 transition">
      ...
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-slate-400 flex items-center gap-1">
            <Clock size={10} /> {formatMs(totalMs)}
          </span>
        </div>
      </div>
      ...
      <div className="flex flex-wrap items-center gap-1 pl-4">
        ...
        >
          {log.status_code}{" "}
          {log.status_code >= 200 && log.status_code < 300 ? "OK" : ""}
        </span>
        <ArrowDown size={10} className="text-slate-600 rotate-[-90deg]" />
      ...
      <div className="mt-2 pl-4 text-[10px] text-slate-600 flex gap-4">
        <span>
          Request ID:{" "}
          <code className="font-mono">{log.request_id?.slice(0, 16)}…</code>
        </span>
        {log.cache_hit && <span className="text-brand-400">Cache HIT</span>}
        {log.retry_count > 0 && (
          <span className="text-amber-400">Retried ×{log.retry_count}</span>
        )}
